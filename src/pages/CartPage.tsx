...
export const CartPage: React.FC = () => {
  ...
  const handleQuantityChange = (item: any, newQuantity: number) => {
    ...
    // Перевіряємо доступний залишок
    const sizeInfo = item.product.sizes?.find((s: any) => s.size === item.selectedSize);
    const maxAvailable = sizeInfo?.stock || 0;

    if (newQuantity > maxAvailable) {
      showToast(`Доступно лише ${maxAvailable} шт. цього розміру`, "error");
      return;
    }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            {cart.items.map((item, index) => {
              ...
              return (
                <div
                  ...
                  className="bg-white rounded-xl shadow-soft border border-accent p-4"
                >
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {/* Попередження про малу кількість */}
                      {maxAvailable < 5 && maxAvailable > 0 && (
                        <p className="text-xs text-error mt-1 flex items-center gap-1">
                          <span className="w-1.5 h-1.5 bg-error rounded-full"></span>
                          Залишилось лише {maxAvailable} шт. цього розміру
                        </p>
                      )}
                      ...
                      <div className="flex justify-between items-center mt-3">
                        <div className="flex items-center border border-accent rounded-lg">
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center hover:bg-accent disabled:opacity-50"
          ...
          <div className="lg:w-96">
            <div className="bg-white rounded-xl shadow-soft border border-accent p-5 sticky top-24">
              ...
              <Button onClick={handleCheckout} className="w-full mt-6 py-3">
                Оформити замовлення
              </Button>
              <Button
                variant="outline"
