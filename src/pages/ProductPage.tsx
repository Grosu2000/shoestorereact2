...
export const ProductPage: React.FC = () => {
  ...
  const { data: products, isLoading } = useProducts({ category, brand, search });
  ...
  return (
    <div className="min-h-screen bg-background py-6 sm:py-8">
      <div className="container mx-auto px-4">
        ...
        {/* Кнопка відкриття фільтрів (мобільна) */}
          ...
          <div className={`lg:col-span-1 ${showFilters ? 'block' : 'hidden lg:block'}`}>
            <div className="bg-white rounded-xl shadow-soft border border-accent p-5 sticky top-24 space-y-5">
              ...
              <form onSubmit={handleSearch} className="space-y-4">
                ...
                />
                <Input
                  placeholder="Категорія (sneakers, boots...)"
                  value={category}
          ...
          <div className="lg:col-span-3">
            {isLoading ? (
              <div className="flex justify-center py-12">
                <div className="w-8 h-8 border-2 border-button border-t-transparent rounded-full animate-spin" />
              </div>
            ) : (
              <>
                <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                  <p className="text-sm text-text/60">
                    Знайдено товарів: {products?.length || 0}
                  </p>
                </div>
