  ...
  return (
    <Link to={`/product/${product.id}`} className="block group">
      <div className="relative bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
        ...
        >
          <svg
            className={`w-5 h-5 ${isWishlisted ? "fill-red-500 text-red-500" : "fill-none text-gray-500"}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="relative overflow-hidden bg-accent/30">
          ...
          {!inStock && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
              <span className="bg-white text-text px-3 py-1 rounded-full text-sm font-medium">
                Немає в наявності
              </span>
            </div>
          )}
        ...
        {/* ІНФОРМАЦІЯ */}
        <div className="p-4">
          <div className="text-xs text-text/50 uppercase tracking-wider mb-1">
            {product.brand}
          </div>
              ...
              <div className="flex items-center gap-1">
                <div className="flex -space-x-1">
                  {displaySizes.map((size) => (
                    <span
                      key={size}
                      className="inline-block w-6 h-6 bg-accent rounded-full text-center text-xs leading-6 font-medium"
                    >
                      {size}
                    </span>
                  ))}
                </div>
                {Object.keys(sizeColorMatrix).length > 3 && (
                  <span className="text-xs text-text/50">
                    +{Object.keys(sizeColorMatrix).length - 3}
                  </span>
                )}
