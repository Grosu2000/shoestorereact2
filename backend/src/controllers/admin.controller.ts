...
export const getAllOrders = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Помилка отримання замовлень" });
  }
};
...
export const updateOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Помилка оновлення статусу" });
  }
};
...
export const getAllProducts = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Помилка отримання товарів" });
  }
};
...
export const createProduct = async (req: Request, res: Response) => {
  try {
    ...
    const {
      ...
      material,
      features,
      discountPercent, // ← ДОДАТИ ЦЮ ЗМІННУ
    } = req.body;
    let discount = 0;
    if (discountPercent) {
      discount =
    ...
    let parsedMatrix = {};
    try {
      parsedMatrix = sizeColorMatrix ? JSON.parse(sizeColorMatrix) : {};
    } catch (e) {
      parsedMatrix = {};
    }
...
export const updateProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.discountPercent) {
      const discount = parseInt(req.body.discountPercent);
      updateData.discountPercent = Math.min(100, Math.max(0, discount));
    }
